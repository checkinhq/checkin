...
func TestDbRepository_Update(t *testing.T) {
	...
	expected.Email = "john.new@doe.com"
...
func newUser(t *testing.T, now time.Time) *user.User {
...
func saveUser(t *testing.T, db *sql.DB, user *user.User) int64 {
	result, err := db.Exec(
		`
		INSERT INTO
			users/*t*/
...
func getUser(t *testing.T, db *sql.DB, id int64) *user.User {
	u := new(user.User)
